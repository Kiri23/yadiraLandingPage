export default function Services2() {
    return (
        <div class="">
            <h3 class={"text-3xl text-center pb-8"}>My values</h3>
            <div class="grid grid-cols-3 border-y-brownBrand border-y-2">
                <div
                    class={"border-r-2 border-r-brownBrand shadow-sm p-10"}
                >
                    <h4 class="text-4xl font-semibold text-gray-800">
                        Empathy
                    </h4>

                    <p class="mt-2 sm:mt-3 text-xl">
                        At Be In peace, we offer a range of services to address
                        your unique mental health and wellness needs. I'm here
                        to guide you on your journey to personal growth and
                        fulfillment.
                    </p>
                </div>
                <div
                    class={"border-r-2 border-r-brownBrand shadow-sm p-10"}
                >
                    <h4 class="text-4xl font-semibold text-gray-800">
                        Compassion
                    </h4>
                    <p class="mt-2 sm:mt-3 text-xl">
                        Personalized one-on-one sessions to address your
                        specific mental health concerns and goals.
                    </p>
                </div>
                <div
                    class={"shadow-sm p-10"}
                >
                    <h4 class="text-4xl font-semibold text-gray-800">
                        Telehealth Services
                    </h4>
                    <p class="mt-2 sm:mt-3 text-xl">
                        Convenient and accessible remote therapy sessions to
                        support your mental health needs.
                    </p>
                </div>
            </div>
        </div>
        // <div class="grid w-[86em] mx-auto border rounded-xl shadow-sm divide-y overflow-hidden sm:flex sm:divide-y-0 sm:divide-x">
        //     <div class="flex flex-col flex-[1_0_0%] mx-16">
        //         <img
        //             class="w-56 h-56 m-auto rounded-t-xl sm:rounded-se-none"
        //             src="/doctor.svg"
        //             alt="Doctor"
        //         />
        //         <div class="p-4 flex-1 md:p-5">
        //             <h3 class="text-lg font-bold text-gray-800">
        //                 Card title
        //             </h3>
        //             <p class="mt-1 text-gray-500">
        //                 This is a wider card with supporting text below as a
        //                 natural lead-in to additional content. This content is a
        //                 little bit longer.
        //             </p>
        //         </div>
        //     </div>

        //     <div class="flex flex-col flex-[1_0_0%] mx-16">
        //         <img
        //             class="w-56 h-56 m-auto rounded-t-xl sm:rounded-se-none"
        //             src="/hospital.svg"
        //             alt="Doctor"
        //         />
        //         <div class="p-4 flex-1 md:p-5">
        //             <h3 class="text-lg font-bold text-gray-800">
        //                 Card title
        //             </h3>
        //             <p class="mt-1 text-gray-500">
        //                 This card has supporting text below as a natural lead-in
        //                 to additional content.
        //             </p>
        //         </div>
        //     </div>

        //     <div class="flex flex-col flex-[1_0_0%] mx-16">
        //         <img
        //             class="w-56 h-56 m-auto rounded-t-xl sm:rounded-se-none"
        //             src="/capsules-solid.svg"
        //             alt="Doctor"
        //         />
        //         <div class="p-4 flex-1 md:p-5">
        //             <h3 class="text-lg font-bold text-gray-800">
        //                 Card title
        //             </h3>
        //             <p class="mt-1 text-gray-500">
        //                 This is a wider card with supporting text below as a
        //                 natural lead-in to additional content. This card has
        //                 even longer content than the first to show that equal
        //                 height action.
        //             </p>
        //         </div>
        //     </div>
        // </div>
    );

    //   <div class="max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-auto">
    //     <div class="grid gap-6 grid-cols-2 sm:gap-12 lg:grid-cols-3 lg:gap-8">
    //       <div class={"border border-gray-200 shadow-sm rounded-xl p-4 md:p-5"}>
    //         <h4 class="text-lg sm:text-xl font-semibold text-gray-800">
    //           Personalized Care for Your Wellbeing
    //         </h4>
    //         <p class="mt-2 sm:mt-3 text-4xl sm:text-6xl font-bold text-blue-600">
    //           At Be In peace, we offer a range of services to address your unique
    //           mental health and wellness needs. I'm here to guide you on your
    //           journey to personal growth and fulfillment.
    //         </p>
    //       </div>
    //       <div class={"border border-gray-200 shadow-sm rounded-xl p-4 md:p-5"}>
    //         <h4 class="text-lg sm:text-xl font-semibold text-gray-800">
    //           Individual Therapy
    //         </h4>
    //         <p class="mt-2 sm:mt-3 text-4xl sm:text-6xl font-bold text-blue-600">
    //           Personalized one-on-one sessions to address your specific mental
    //           health concerns and goals.
    //         </p>
    //         <p class="mt-1 text-gray-500">partner with Preline</p>
    //       </div>
    //       <div class={"border border-gray-200 shadow-sm rounded-xl p-4 md:p-5"}>
    //         <h4 class="text-lg sm:text-xl font-semibold text-gray-800">
    //           Telehealth Services
    //         </h4>
    //         <p class="mt-2 sm:mt-3 text-4xl sm:text-6xl font-bold text-blue-600">
    //           Convenient and accessible remote therapy sessions to support your
    //           mental health needs.
    //         </p>
    //         <p class="mt-1 text-gray-500">this year alone</p>
    //       </div>
    //     </div>
    //   </div>
    // );
}
/**
   * Depression Treatment
   * Personalized care to help you overcome life's emotional hurdles
                and achieve mental well-being.
   */
